...
new Promise((resolve, reject) => {
  ...
  const request = http.request(options, (response) => { // response is a parameter
    ...
    response.on('error', reject); // node (the environment) sends the error
    ...
    response.on('data', (chunk) => { // node (the environment) sends the data
      data += chunk;
    });
    response.on('end', () => { // node (the environment) sends the end
      resolve(data);
    });
  });
...
// order of stuff: line 52, then line 69 (ayy), THEN line 53
